...
class semiBatchGA0 extends solutionOperation {
    constructor(sysAlgoConfig) {
        super(sysAlgoConfig);
        const config = sysAlgoConfig;
        this.ans = config.ans
        ...
        this.a = 0.2, //A constant value. The more a, the more focus on reconf cost. ***This is applicable just for serial semi-batch GA.***
        this.initialPlacementSize = 2, //The number of all users divided by this.initialPlacementSize. 50% is considered as initial placement.
    ...
    reconfigurationCost(prev, solutions)
    {
        let reconfCosts = [];
    ...
    solutionAnalyser(prev, solution)
    {
        const cost = this.GA.calculateAll(solution)
        const load = this.GA.loadCalculator(solution);
        const confCost = this.reconfigurationCost(prev, [solution])
        return {
            totalResponseTime: cost['ResponseTime'],
            platformReliability: cost['PlatformReliability'],
            serviceReliability: cost['ServiceReliability'],
    ...
    quality_t1(prev, solutions)
    {
        ...
        {
            qul.push((1 - this.a)*normalized_cost_pacement[i] + this.a*normalized_cost_reconfiguration[i]);
        }
        return qul;
    }
    ...
    initialPlacement()
    {
        ...
        const initialPlacement = this.GA.run(this.initialSolutions_t0(this.numPopulation), 2)
    ...
    run()
    {
        ...
        let cost = this.quality_t1(initPlacement, population)
        let costInfoCurrent;
        ...
        for (let i = 0; i < this.iteration; i++)
        {
            ...
            cost = this.quality_t1(initPlacement, mutationPopulation)
        ...
        //console.log(costInfoCurrent['bestQuality'])
        console.log(this.solutionAnalyser(initPlacement, costInfoCurrent['bestSolution']));
    }
}
